...
DROP POLICY IF EXISTS "Authenticated users can view aircraft for marketplace" ON aircraft;
...
-- The existing "Operators can update their own aircraft" policy already handles operator access
-- We don't need to duplicate it with a separate SELECT policy
...
DROP POLICY IF EXISTS "Authenticated users can view crew availability" ON crew_availability;
...
DROP POLICY IF EXISTS "Limited crew certification access" ON crew_certifications;
...
USING (
  ...
  EXISTS (
    ...
    AND (hr.broker_id = (select auth.uid()) OR EXISTS (
      SELECT 1 FROM profiles p WHERE p.user_id = (select auth.uid()) AND p.role = 'operator'
    ))
