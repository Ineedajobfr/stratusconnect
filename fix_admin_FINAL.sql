-- FINAL FIX for Admin Console - Handles username requirement
...
-- 7. Make Stratuscharters@gmail.com admin (with username!)
UPDATE public.users
SET 
  role = 'admin', 
  status = 'active', 
...
-- If doesn't exist, create with username
INSERT INTO public.users (
  id, email, username, full_name, role, status, verification_status, 
...
-- 8. Add test users WITH usernames
INSERT INTO public.users (
  id, email, username, full_name, role, status, verification_status, 
...
SELECT 
  ...
  COUNT(*) FILTER (WHERE last_login_at IS NOT NULL) as active_today
FROM public.users;
