# STRATUSCONNECT BOT TESTING SYSTEM - PowerShell Version
...
function Test-Bot {
    ...
    foreach ($Terminal in $Terminals) {
        ...
        try {
            ...
            if ($TestResult.Success) {
                Write-Host "✅ Bot $BotId`: $($Terminal.Name) - SUCCESS ($($LoadTime.ToString('F0'))ms, $($Response.Content.Length) bytes)" -ForegroundColor Green
            } else {
                Write-Host "❌ Bot $BotId`: $($Terminal.Name) - FAILED (Status: $($Response.StatusCode), Content: $($Response.Content.Length) bytes)" -ForegroundColor Red
            }
    ...
    Write-Host "🏁 Bot $BotId`: Test complete - $($SuccessRate.ToString('P1')) success rate ($($TotalTime.TotalSeconds.ToString('F1'))s)" -ForegroundColor Cyan
